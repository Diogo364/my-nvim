vim.keymap.set("n", "<leader>fd", function()
    ...
    vim.ui.input({ prompt = string.format("delete %s [y/n]: ", curr_file) }, function(input)
        should_delete = input
    end)
...
vim.keymap.set("n", "<leader>fc", function()
    ...
    vim.ui.input(
        { prompt = string.format("copy %s to:", curr_file), completion = "file", default = curr_file },
        function(input)
            new_filename = input
        end
    )
...
vim.keymap.set("n", "<leader>fm", function()
    ...
    local new_filename = nil
    vim.ui.input(
        { prompt = string.format("Move %s: ", curr_file), completion = "file", default = curr_file },
        function(input)
            new_filename = input
        end
    )
...
vim.keymap.set("n", "<leader>fn", function()
    ...
    local new_f = nil
    vim.ui.input({ prompt = "new file name: ", default = cwd, completion = "file" }, function(input)
        new_f = input
    end)
    if new_f == nil then
        print("Canceled")
    ...
    vim.cmd({ cmd = "e", args = { new_f } })
    vim.cmd({ cmd = "w" })
    print(string.format("Created %s", new_f))
end, { desc = "Create a new file" })

vim.keymap.set("n", "<leader><leader>n", ":e /tmp/tmp.txt<cr>", { desc = "Open temporary file" })

vim.keymap.set("n", "<leader><leader>x", "<cmd>!chmod +x %<cr>", { silent = true, desc = "Make file executable" })
